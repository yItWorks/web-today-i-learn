...
VALUES (13, "2026-03-06", "09:31:00", "18:01:00");
```
...
검프는 SQL이 익숙지 않아**`crew`**테이블에서 먼저 닉네임을 검색하고 해당 아이디 값을 찾아 직접 WHERE문에서**`crew_id`**항목의 값을 수동으로 입력해서 출석 기록을 조회했다. 그런데*
*`crew`**테이블에서**`crew_id`**를 기준으로**`nickname`**필드 값을 가져와서 함께 조회할 수도 있지 않을까?
...
검프는 SQL이 익숙지 않아**`crew`**테이블에서 먼저 닉네임을 검색하고 해당 아이디 값을 찾아 직접 WHERE문에서**`crew_id`**항목의 값을 수동으로 입력했다. 그런데 nickname을 입력하면 이를
기준으로 쿼리문을 처리할 수도 있지 않을까?
...
FROM attendance
         JOIN crew
              ON attendance.crew_id = crew.crew_id
WHERE attendance.attendance_date = "2026-03-05"
ORDER BY attendance.end_time DESC
LIMIT 1;
```
...
```sql
SELECT crew_id, COUNT(start_time) AS present_start_time_count
FROM attendance
WHERE start_time
GROUP BY crew_id;
...
```sql
SELECT attendance_date, COUNT(crew_id) AS attendance_crew_count
FROM attendance
